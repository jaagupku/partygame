...
type Widen<T> =
	...
	: T extends readonly (infer Item)[] ? Widen<Item>[]
...
const en = defineMessages({
		...
		finale: {
			title: 'Final Results',
			topPlayersRevealed: 'Top players revealed',
		...
		editor: {
			...
			timer: 'Timer',
			timerHelp: 'Set the pace of the question and decide whether the step should close automatically.',
...
const et: Messages = {
		...
		create: {
			title: 'Loo mäng',
			subtitle:
				'Vali definitsioon, vaata see üle ja otsusta, kas mängu juhib mängujuhi telefon.',
		...
		definitions: {
			...
			couldNotLoadDefinition: (definitionId) =>
				`Definitsiooni "${definitionId}" ei saanud laadida.`,
		...
		finale: {
			title: 'Lõpptulemused',
			topPlayersRevealed: 'Parimad mängijad on välja toodud',
		...
		editor: {
			...
			timer: 'Taimer',
			timerHelp:
				'Määra küsimuse tempo ja otsusta, kas samm peaks automaatselt sulguma.',
			...
			deleteStepMessage:
				'See samm eemaldatakse definitsioonist ja seda ei saa redaktoris taastada.',
